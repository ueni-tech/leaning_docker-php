<?php
$dsn = 'mysql:host=mysql;dbname=php_db_app;charset=utf8mb4';
$user = 'root';
$password = 'password';
...
if(isset($_POST['submit'])){
  try {
    ...
    $stmt_update->bindValue(':vendor_code', $_POST['vendor_code'], PDO::PARAM_STR);
    ...
    <article class="registration">
      ...
      <form action="update.php?id=<?= $_GET['id'] ?>" method="post" class="registration-form">
        <div>
          ...
          <input type="number" name="product_code" min="0" max="100000000" value="<?= $product['product_code'] ?>" required>

          <label for="product_name">商品名</label>
          <input type="text" name="product_name" maxlength="50" value="<?= $product['product_name'] ?>" required>

          <label for=" price">単価</label>
          <input type="number" name="price" min="0" max="100000000" value="<?= $product['price'] ?>" required>

          <label for=" stock_quantity">在庫数</label>
          <input type="number" name="stock_quantity" min="0" max="100000000" value="<?= $product['stock_quantity'] ?>" required>
